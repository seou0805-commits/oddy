...
import { playGimmick, stopGimmick } from "./useThreeScene"
...
export function useCarousel(modelsRef: React.MutableRefObject<(THREE.Group | null)[]>) {
  ...
  useEffect(() => {
    ...
    ;[0, 1, 2].forEach((i) => stopGimmick(i))
    playGimmick(frontIdx)
  }, [frontIdx])
  ...
  const rotate = useCallback(
    (dir: "left" | "right") => {
      ...
      models.forEach((group, modelIdx) => {
        ...
        tl.to(group.position, { x: pos.x, z: pos.z, duration: 0.55, ease: "power2.inOut" }, 0)
        tl.to(group.scale, { x: pos.scale, y: pos.scale, z: pos.scale, duration: 0.55, ease: "power2.inOut" }, 0)
